...
def fgrain(clip, iterations=800, size=0.5, deviation=0.0, blur_strength=0.9, opacity=0.1):
    # checks
    if clip.format.color_family not in (vs.YUV, vs.GRAY):
        raise ValueError("Clip must be in YUV or GRAY format.")
    ...
    # convert to float
    clipY_format = clipY.format.id
    if clipY.format.id != vs.GRAYS:
        clipY = core.resize.Point(clipY, format=vs.GRAYS)

    # add grain
    clipYgrain = core.akarin.PropExpr(clipY, lambda: dict(FGRAIN_SEED_OFFSET="N width *"))
    clipYgrain = core.fgrain_cuda.Add(clipYgrain, num_iterations=iterations, grain_radius_mean=size, grain_radius_std=deviation, sigma=blur_strength)
    clipYgrain = core.std.Merge(clipY, clipYgrain, weight=opacity)

    # convert back
    if clipYgrain.format.id != clipY_format:
        clipYgrain = core.resize.Point(clipYgrain, format=clipY_format)

    # merge
    if clip.format.color_family == vs.YUV:
        return core.std.ShufflePlanes(clips=[clipYgrain, clip, clip], planes=[0, 1, 2], colorfamily=vs.YUV, prop_src=clip)
    return clipYgrain
